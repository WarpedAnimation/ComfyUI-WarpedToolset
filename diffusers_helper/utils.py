...
def soft_append_bcthw(history, current, overlap=0):
    ...
    print("soft_append_bcthw: history Shape: {}".format(history.shape))
    print("soft_append_bcthw: history Shape: {}".format(current.shape))
    print("soft_append_bcthw: overlap: {}".format(overlap))

    weights = torch.linspace(1, 0, overlap, dtype=history.dtype, device=history.device).view(1, 1, -1, 1, 1)
    
    print("weights: Shape: {}".format(weights.shape))
    print("weights: history[:, :, -overlap:] Shape: {}".format(history[:, :, -overlap:].shape))
    print("weights: current[:, :, overlap:] Shape: {}".format(current[:, :, overlap:].shape))
